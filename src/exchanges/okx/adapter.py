...
class OkxAdapter(BaseExchange):
    ...
    def _precision(value: str) -> int:
        if not value or "." not in value: return 0
        ...
        return {
            ...
            "markets": {
                ...
                MarketType.LINEAR: {
                    ...
                    "open_interest":    {"rest": True, "ws": False, "intervals": OI_INTERVALS},
                    "funding_rate":     {"rest": True, "ws": False},
                    "long_short_ratio": {"rest": True, "ws": False, "intervals": LSR_INTERVALS},
    ...
    async def get_exchange_info(self, market_type: MarketType) -> List[SymbolInfo]:
        ...
        results = []
        for s in data:
            if s.get("state") != "live": continue
            if market_type == MarketType.LINEAR and s.get("ctType") != "linear": continue
            if market_type == MarketType.INVERSE and s.get("ctType") != "inverse": continue
    ...
    async def get_ticker(self, market_type: MarketType, symbol: str) -> Ticker:
        ...
        if not data: raise ValueError(f"Symbol {api_symbol} not found")
        t = data[0]
    ...
    async def get_mark_price(self, market_type: MarketType, symbol: str) -> MarkPrice:
        if market_type == MarketType.SPOT: raise NotImplementedError("Mark Price not available for Spot")
        api_symbol = self.get_api_symbol(symbol, market_type)
        mark_data = await self._make_request("GET", "/api/v5/public/mark-price", {"instType": "SWAP", "instId": api_symbol})
        if not mark_data: raise ValueError(f"Mark price not found for {api_symbol}")
        m = mark_data[0]
        ...
        try:
            ...
            if i_data: index_price = float(i_data[0].get("idxPx") or 0)
        except (httpx.HTTPStatusError, ValueError):
            pass
    ...
    async def get_orderbook(self, market_type: MarketType, symbol: str, depth: int = 20) -> OrderBook:
        api_symbol = self.get_api_symbol(symbol, market_type)
        sz = max(1, min(depth, 400))
        data = await self._make_request("GET", "/api/v5/market/books", {"instId": api_symbol, "sz": sz})
        if not data: raise ValueError(f"Orderbook not found for {api_symbol}")
        b = data[0]
        return OrderBook(
    ...
    async def get_trades(self, market_type: MarketType, symbol: str, limit: int = 100) -> List[Trade]:
        api_symbol = self.get_api_symbol(symbol, market_type)
        data = await self._make_request("GET", "/api/v5/market/trades", {"instId": api_symbol, "limit": min(limit, 500)})
        return [Trade(
            id=str(t["tradeId"]),
            price=float(t["px"]),
            qty=float(t["sz"]),
    ...
    async def get_agg_trades(self, market_type: MarketType, symbol: str, start_time: Optional[int] = None, limit: int = 500) -> List[AggTrade]:
        raise NotImplementedError("OKX has no aggregated trades endpoint")
    ...
    async def get_candles(self, market_type: MarketType, symbol: str, interval: str, start_time: Optional[int] = None, limit: int = 100) -> List[Candle]:
        ...
        async def fetch_recent(end_ts, l):
            ...
            p = {"instId": api_symbol, "bar": bar, "limit": min(l, 300)}
            if anchor: p["after"] = str(anchor + 1)
        ...
        if start_time is None and limit <= 300:
            return (await fetch_recent(None, limit))[-limit:]

        recent_batch = await fetch_recent(None, min(limit, 300))
    ...
    async def get_funding_rate(self, market_type: MarketType, symbol: str, start_time: Optional[int] = None, limit: int = 100) -> List[FundingRate]:
        ...
        async def fetch_backward(end_ts, l):
            anchor = end_ts if end_ts is not None else start_time
            p = {"instId": api_symbol, "limit": min(l, 100)}
            if anchor: p["after"] = str(anchor + 1)
            return _parse(await self._make_request("GET", "/api/v5/public/funding-rate-history", p), model_sym)

        if start_time is not None or limit > 100:
            return await self._paginate_backwards(fetch_backward, limit, 100)
        return await fetch_backward(None, limit)
    ...
    async def get_open_interest(self, market_type: MarketType, symbol: str, period: str = "1h", start_time: Optional[int] = None, limit: int = 30) -> List[OpenInterest]:
        if market_type == MarketType.SPOT: raise NotImplementedError("Open Interest not available for Spot")
        api_symbol = self.get_api_symbol(symbol, market_type)
        model_sym = self.get_model_symbol(api_symbol, market_type)
            ...
            for r in rows:
                ...
                try:
                    ...
                    out.append(OpenInterest(
                        symbol=model_sym, open_interest=oi, value_usd=usd, timestamp=ts
                    ))
                except (ValueError, TypeError):
                    continue
            return sorted(out, key=lambda x: x.timestamp)
        ...
        async def fetch_backward(end_ts, l):
            anchor = end_ts if end_ts is not None else start_time
            p = {"instId": api_symbol, "period": bar, "limit": min(l, 100)}
            if anchor: p["end"] = str(anchor)
    ...
    async def get_long_short_ratio(self, market_type: MarketType, symbol: str, period: str = "5m", start_time: Optional[int] = None, limit: int = 30) -> List[LongShortRatio]:
        if market_type == MarketType.SPOT: raise NotImplementedError("L/S Ratio not available for Spot")
            ...
            for r in rows:
                ...
                try:
                    ...
                    out.append(LongShortRatio(
                        symbol=model_sym, ratio=ratio,
                        long_account=long_acct, short_account=short_acct, timestamp=ts
                    ))
                except (ValueError, TypeError):
                    continue
            return sorted(out, key=lambda x: x.timestamp)
        ...
        async def fetch_backward(end_ts, l):
            anchor = end_ts if end_ts is not None else start_time
            p = {"ccy": ccy, "period": bar, "limit": min(l, 100)}
            if anchor: p["end"] = str(anchor)
    ...
    async def get_liquidations(self, market_type: MarketType, symbol: str, start_time: Optional[int] = None, limit: int = 100) -> List[Liquidation]:
        ...
        def _parse(rows):
            out = []
            for entry in rows:
                if entry.get("instId") != api_symbol: continue
        ...
        async def fetch_backward(end_ts, l):
            anchor = end_ts if end_ts is not None else start_time
            p = {"instType": "SWAP", "uly": uly, "state": "filled", "limit": min(l, 100)}
            if anchor: p["after"] = str(anchor + 1)
            try:
